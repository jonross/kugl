...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            """select xyz from my.pods a join his.nodes b""",
            ["my.pods", "his.nodes"],
            None,
        ),
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "SELECT * FROM oh@my.stuff",
            "invalid schema name in 'oh@my.stuff' -- must contain",
        ),
        (
            "SELECT * FROM my.@stuff",
            "invalid table name in 'my.@stuff' -- must contain",
        ),
        (
            "SELECT * FROM main.stuff",
            "invalid schema name, must not be 'main', 'temp', or 'init'",
        ),
        (
            "SELECT * FROM temp.stuff",
            "invalid schema name, must not be 'main', 'temp', or 'init'",
        ),
        (
            "SELECT * FROM init.stuff",
            "invalid schema name, must not be 'main', 'temp', or 'init'",
        ),
    ],
)
