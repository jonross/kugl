...
def test_print_entire_schema(test_home, capsys):
    ...
    assert (
        "status     text     job status, one of 'Running', 'Complete', 'Suspended', 'Failed', 'Unknown'"
        in out
    )
    assert "cpu_alloc  real     allocatable CPUs, from status.allocatable" in out
    assert (
        "creation_ts  integer  creation timestamp in epoch seconds, from metadata.creationTimestamp"
        in out
    )
