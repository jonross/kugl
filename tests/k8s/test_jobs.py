...
def test_job_status(test_home):
    ...
    assert_query(
        "SELECT name, uid, namespace, status FROM jobs ORDER BY 1",
        """
...
def test_job_labels(test_home):
    ...
    assert_query(
        "SELECT job_uid, key, value FROM job_labels ORDER BY 2, 1",
        """
        job_uid    key    value
        uid-job-2  a      b
        uid-job-2  c      d
...
def test_label_parents(test_home):
    ...
    assert_query(
        """SELECT job_username, pod_username FROM job_users""",
        """
