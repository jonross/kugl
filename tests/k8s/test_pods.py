...
def test_by_cpu(test_home, capsys):
    kubectl_response(
        "pods",
        {
            "items": [
                ...
                make_pod(
                    "pod-4",
                    containers=[Container(requests=CGM(cpu="2000m", mem="10M"))],
                ),
    ...
    with features_debugged("extract,fetch"):
        assert_query(
            "SELECT name, status FROM pods WHERE cpu_req > 1 ORDER BY name",
            """
            name    status
            pod-3   Init:3
            pod-4   Init:4
        ...
        "pods",
        {
            "items": [
                ...
                make_pod(
                    "pod-3",
                    node_name="joe",
                    creation_ts=UNIT_TEST_TIMEBASE + 60,
                    deletion_ts=UNIT_TEST_TIMEBASE + 600,
                ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [
                Container(
                    requests=CGM(cpu=1, mem="1Mi", gpu=3),
                    limits=CGM(cpu=1, mem="1Mi", gpu=3),
                )
            ],
            [[1, 1, 1 << 20, 1 << 20, 3, 3]],
        ),
...
def test_resource_summing(test_home, containers, expected):
    ...
    assert_query(
        "SELECT cpu_req, cpu_lim, mem_req, mem_lim, gpu_req, gpu_lim FROM pods",
        expected,
    )
    ...
    assert_query(
        "SELECT cpu_req, cpu_lim, mem_req, mem_lim, gpu_req, gpu_lim FROM jobs",
        expected,
    )
...
def test_pod_labels(test_home):
    ...
    assert_query(
        "SELECT pod_uid, key, value FROM pod_labels ORDER BY 2, 1",
        """
        pod_uid    key    value
        uid-pod-2  a      b
        uid-pod-2  c      d
