...
def test_regular_only(test_home):
    ...
    _setup([
        make_pod("pod-1", containers=[
            Container(name="main", requests=CGM(cpu=2, mem="20M"), limits=CGM(cpu=2, mem="20M")),
...
def test_init_containers(test_home):
    ...
    _setup([
        make_pod(
            "pod-1",
            containers=[Container(name="app", requests=CGM(cpu=2, mem="20M"), limits=CGM(cpu=2, mem="20M"))],
            init_containers=[Container(name="setup", requests=CGM(cpu=1, mem="10M"), limits=CGM(cpu=1, mem="10M"))],
        ),
    ])
    assert_query(
...
def test_resources_not_summed(test_home):
    ...
    _setup([
        make_pod("pod-1", containers=[
            Container(name="big", requests=CGM(cpu=4, mem="40M"), limits=CGM(cpu=4, mem="40M")),
            Container(name="small", requests=CGM(cpu=1, mem="10M"), limits=CGM(cpu=1, mem="10M")),
        ]),
    ])
    assert_query(
