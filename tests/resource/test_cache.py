...
def test_cache(test_home, capsys):
    ...
    events = ResourceRef(
        mock_schema, KubernetesResource(name="events", cacheable=False, namespaced=True)
    )
    all_res = {pods, jobs, nodes, events}

    for r in all_res:
        r.resource.handle_cli_options(SimpleNamespace(namespace="foo", all=False, context=None))
